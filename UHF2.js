...
$(document).ready(function () {
    ...
    hold_trigger.mousedown(function () {
        if (power == 0) {
            ...
            hold_trigger.bind('mouseup mouseleave', function () {
    ...
    $('body').on('click', '#up', up);
    $('body').on('click', '#down', down);

    $('#numbers td').each(dialpad);
    $('#duress').on('click', duress);
    ...
    //microphone
    $('#micduress').on('click', duress);
    $('#ptt').on('click', transmit);
...
function up() {
    if (power == 1 && menumode == 0) { menumodedisplay(); }
    //
...
function menumodedisplay() {
    ...
    //test tooltip
    tooltips(arguments.callee.name);
        ...
        if (power == 1) {
            ...
            if (dialcode = dialedstring.match(dialmatch)) {
                ...
                if (channelinput == -1 | channelinput > channels.length) {
                    ...
                    $('#dialed').html("");
                    // console.log("the value of i is: " + i + " The channel is: " + channels[i].ChannelName + " The chan number is: " + channels[i].ChannelNumber);
                    tooltips("dialpad", "<p>Well Done. The channel input has matched to " + channels[i].ChannelNumber);
                };
...
function usermenuright() {
    ...
    // console.log("m is " + m);
    tooltips(arguments.callee.name, usermenu[m].mainvalue);
};
...
function setupmenuup() {
    ...
    // console.log("m is " + m);
    tooltips(arguments.callee.name, setupmenu[m].menutitle);
};
...
function menucommands(menuname, menuvalue) {
    ...
    if (menuname == "Key beeps") { /* console.log("Dual Watch updated") */ };
...
function f1() {
    if (power == 1) {
        ...
        //if ($('#scan').html("Scan") == "Scan" && $('#scan').css('display') == 'visible') {
...
function tooltips(functionname, textinput) {
    ...
    if ($('#console')) {
        ...
        if (functionname == "transmit") { details = "Transmit/Receive (Simulation):<p>What you are seeing is a simulation of what you will see and hear if you pressed the PTT button whilst tuned into a repeater channel<p>Eg, SES CH 1 through to 15, and CB 1-8.<p>Notice the <b><span style=\"font-family:Webdings\">&#88;</span></b> icon is displayed on the screen to notify you a transmission is received, a signal icon ( <img src=\"images/signalmeter.gif\" />) is displayed representing the strength of the transmission. Full signal strength is indicated here." };
